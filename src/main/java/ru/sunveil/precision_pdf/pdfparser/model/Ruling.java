...
public class Ruling extends Line2D.Float {
    ...
    public static Map<Point2D, Ruling[]> findIntersections(List<Ruling> horizontals, List<Ruling> verticals) {
        ...
        Collections.sort(sos, new Comparator<SortObject>() {
            ...
            public int compare(SortObject a, SortObject b) {
                int rv;
                if (GeometryUtils.feq(a.position, b.position)) {
    ...
    public boolean isVertical() {
        return this.length() > 0 && GeometryUtils.feq(super.getP1().getX(), super.getP2().getX());
    }

    public boolean isHorizontal() {
        return this.length() > 0 && GeometryUtils.feq(super.getP1().getY(), super.getP2().getY());
    }
    ...
    public void normalize() {
        ...
        if (GeometryUtils.within(angle, 0, 1) || GeometryUtils.within(angle, 180, 1)) { // almost horizontal
            this.setLine(this.x1, this.y1, this.x2, this.y1);
        }
        else if (GeometryUtils.within(angle, 90, 1) || GeometryUtils.within(angle, 270, 1)) { // almost vertical
